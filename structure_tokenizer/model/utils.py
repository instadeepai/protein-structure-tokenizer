...
def batched_gather_np(params, indices):
    """Implements a numpy equivalent of `tf.gather` with `axis=0` and `batch_dims=1`."""
    take_fn = lambda p, i: np.take(p[0], i[0], axis=0)
    params_list = np.split(params.astype(np.float32), params.shape[0], axis=0)
    indices_list = np.split(indices, indices.shape[0], axis=0)
    ret = list(
        map(take_fn, params_list, indices_list)
    )
    return np.array(ret)
...
def build_initializer(
    distribution: str, scale: float, mode: str = "fan_avg"
