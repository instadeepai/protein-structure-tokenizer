...
from structure_tokenizer.model import r3, utils
...
def atom14_to_atom37(
    atom14_data: jnp.ndarray, aatype: jnp.ndarray  # (N, 14, ...), (N,)
) -> jnp.ndarray:  # (N, 37, ...)
...
def atom37_to_frames(
    ...
    # shape (num_restypes, num_rigidgroups, 3_atoms): (21, 4, 3)
    restype_rigidgroup_base_atom_names = np.full([21, 4, 3], "", dtype=object)
    ...
    # BG: we dont need these for backbone
    # # 4,5,6,7: 'chi1,2,3,4-group'
    # for restype, restype_letter in enumerate(residue_constants.restypes):
    ...
    restype_rigidgroup_mask = np.zeros([21, 4], dtype=np.float32)
    restype_rigidgroup_mask[:, 0] = 1
    restype_rigidgroup_mask[:, 3] = 1
    # BG: we dont need that for backbone
    ...
    # shape (N, 4, 3)
    residx_rigidgroup_base_atom37_idx = np.take(
        restype_rigidgroup_base_atom37_idx, aatype, axis=0
    )

    # Gather the base atom positions for each rigid group.

    base_atom_pos = utils.batched_gather_np(
        all_atom_positions, residx_rigidgroup_base_atom37_idx
    )
    ...
    # (N, 4)
    group_exists = np.take(restype_rigidgroup_mask, aatype, axis=0)

    # Compute a mask whether ground truth exists for the group

    gt_atoms_exist = utils.batched_gather_np(
        all_atom_mask, residx_rigidgroup_base_atom37_idx
    )

    gt_exists = np.min(gt_atoms_exist, axis=-1) * group_exists  # (N, 4)
    ...
    gt_exists = np.reshape(gt_exists, aatype_in_shape + (4,))
    group_exists = np.reshape(group_exists, aatype_in_shape + (4,))
    gt_frames_flat12 = np.reshape(gt_frames_flat12, aatype_in_shape + (4, 12))

    return {
        "rigidgroups_gt_frames": gt_frames_flat12,  # (..., 4, 12)
        "rigidgroups_gt_exists": gt_exists,  # (..., 4)
        "rigidgroups_group_exists": group_exists,  # (..., 4)
    }
...
def torsion_angles_to_frames(
    ...
    torsion_angles_sin_cos: jnp.ndarray,  # (N, 3, 2)
) -> r3.Rigids:  # (N, 4)
    """Compute rigid group frames from torsion angles.
    We only consider the backbone rigid groups here
    ...
    Args:
      ...
      torsion_angles_sin_cos: sin and cosine of the 3 bb torsion angles
    Returns:
    ...
    # all_rots are r3.Rots with shape (N, 4)
    all_rots = r3.Rots(
        ones,
        zeros,
        zeros,
    ...
    all_frames_to_backb = r3.rigids_mul_rots(default_frames, all_rots)
...
def frames_and_literature_positions_to_atom14_pos(
    # aatype: jnp.ndarray, all_frames_to_global: r3.Rigids  # (N)  # (N, 4)
    all_frames_to_global: r3.Rigids,  # (N, 4)
) -> r3.Vecs:  # (N, 14)
    ...
    num_residues = all_frames_to_global.rot.xx.shape[0]

    # Pick the appropriate transform for every atom.
    # residx_to_group_idx = utils.batched_gather(
    #     residue_constants.restype_atom14_to_rigid_group, aatype
    # )
    atom14_to_group_idx = jnp.repeat(
        residue_constants.atom14_to_rigid_group, num_residues, 0
    )
    group_mask = jax.nn.one_hot(atom14_to_group_idx, num_classes=4)  # shape (N, 14, 4)
    ...
    lit_positions = r3.vecs_from_tensor(
        jnp.repeat(residue_constants.atom14_rigid_group_positions, num_residues, 0)
    )
    ...
    mask = jnp.repeat(residue_constants.atom14_mask, num_residues, 0)
